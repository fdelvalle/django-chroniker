...
class MonitorAdmin(admin.ModelAdmin):
    ...
    def status(self, obj):
        if obj.is_running:
            ...
            temp = format_html('<img src="' + settings.STATIC_URL + 'admin/img/icon-unknown.svg" alt="%(help_text)s" title="%(help_text)s" />')
        elif obj.last_run_successful:
            help_text = 'All checks passed.'
            temp = format_html('<img src="' + settings.STATIC_URL + 'admin/img/icon-yes.svg" alt="%(help_text)s" title="%(help_text)s" />')
        else:
            help_text = 'Requires attention.'
            temp = format_html('<img src="' + settings.STATIC_URL + 'admin/img/icon-no.svg" alt="%(help_text)s" title="%(help_text)s" />')
